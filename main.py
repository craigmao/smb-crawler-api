"""
SMB 情报爬虫 API v4.2 — 国内新闻聚合架构

策略: 36kr RSS(主力) + 虎嗅RSS + 搜狗微信 + B站
- ECS部署在中国大陆，Google News/Bing/百度新闻均不可直接抓取
- 使用国内可达的RSS源(36kr、虎嗅) + 搜狗微信 + B站API
...
# 行业相关性关键词 (用于RSS筛选)
INDUSTRY_KW = [
    "家居", "装修", "设计", "定制", "AI", "数字化", "家装", "建材",
    "智能家居", "SaaS", "云", "3D", "BIM", "家具", "整装", "软装",
    "橱柜", "衣柜", "木门", "瓷砖", "卫浴", "全屋", "酷家乐",
    "三维家", "躺平", "欧派", "索菲亚", "尚品", "志邦", "金牌",
    "我乐", "好莱客", "皮阿诺", "门窗", "地板", "涂料",
    "渲染", "CAD", "VR", "数字孪生", "精装", "家电",
]
...
def is_relevant(text: str) -> bool:
    ...
    if not text:
        return False
    lower = text.lower()
    return any(kw.lower() in lower for kw in INDUSTRY_KW)


# ============================================================
# 数据源 1: 36kr RSS (科技商业新闻 — 主力)
# ============================================================
...
# ============================================================
# 数据源 2: 虎嗅 RSS (科技商业深度 — 新增)
# ============================================================
async def fetch_huxiu_rss() -> list:
    """虎嗅 RSS — 科技商业深度文章，筛选行业相关"""
    items = []
    try:
        async with httpx.AsyncClient(timeout=10, follow_redirects=True) as c:
            r = await c.get("https://www.huxiu.com/rss/0.xml", headers={"User-Agent": UA})
            if r.status_code == 200:
                root = ET.fromstring(r.text)
                for item in root.findall(".//item"):
...
async def api_all():
    ...
    names = ["36kr", "huxiu", "wechat", "bilibili"]
    all_items = []
    report = {}
...
async def health():
    return {
        "status": "ok",
        "version": "4.2",
        "sources": ["36kr", "huxiu", "wechat", "bilibili"],
